...
const RankingPage = () => {
  ...
  useEffect(() => {
    fetch("/api/ranking") // Node.js에서 데이터를 제공하는 엔드포인트-> 추후 확정
    ...
    const mockData = {
      data: {
        rankedList: [
          ...
          { rankNum: 2, name: "B Doe", totalPnt: 200 },
          { rankNum: 3, name: "A Doe", totalPnt: 150 },
        ],
      },
    };
    ...
    <div className="ranking-container">
      ...
      <div className="ranking-list">
        {rankings.map((rank) => (
          <div key={rank.rankNum} className="ranking-item">
            <span className="rank-number">{rank.rankNum}</span>
            <span className="rank-name">방구뿡 {rank.name}</span>
            <span className="rank-points">{rank.totalPnt} P</span>
          </div>
        ))}
      </div>
